...
Ns3GymEnv::Ns3GymEnv ()
{
    ...
    m_timeWindow = 0.5;         // Time window for calculating long-term reward
...
Ns3GymEnv::GetReward()
{
    ...
    // 累积网络性能指标，准备计算长期奖励
    m_sumPdr += m_pdr;
    m_sumDelay += m_delaySum;
    m_sumPlr += m_plr;
    m_observationCount++;

    // 如果时间窗口结束，计算长期奖励
    if (m_simTime - m_lastRewardTime >= m_timeWindow) {
        // 计算时间窗口内的平均性能指标
        float avgPdr = m_sumPdr / m_observationCount;
        float avgDelay = m_sumDelay / m_observationCount;
        float avgPlr = m_sumPlr / m_observationCount;

        // 判断长期防守的成功与失败
        if (avgPdr > 0.9) {
            reward += 100.0;  // PDR 高于 90%，给予正奖励
        } else {
            reward -= 50.0;   // PDR 低于 90%，给予负奖励
        }

        if (avgDelay < 1.0) {
            reward += 50.0;   // 延迟低于 1 秒，给予正奖励
        } else {
            reward -= 30.0;   // 延迟超过 1 秒，给予负奖励
        }

        if (avgPlr < 0.1) {
            reward += 30.0;   // 丢包率低于 10%，给予正奖励
        } else {
            reward -= 20.0;   // 丢包率高于 10%，给予负奖励
        }
        ...
        m_sumPlr = 0.0;
        m_observationCount = 0;
        m_lastRewardTime = m_simTime;  // 更新最后一次计算奖励的时间
    }
...
Ns3GymEnv::ExecuteActions(Ptr<OpenGymDataContainer> action)
{
    ...
    // std::cout << "m_srcAddr: " << m_srcAddr << std::endl;
    // std::cout << "Whitelist: ";
    ...
    // // Identify defend type
    // if (defendType == 0) {
    //     // No defense
    //     m_defendSuccess = 0;
    //     std::cout << "No defense required" << std::endl;
    // } else if (defendType == 1 && m_whitelist.find(m_srcAddr) != m_whitelist.end()) {
    //     // Source address in white list, failed attempts
    //     m_defendSuccess = 1;
    //     std::cout << "Defense failed, source address is in whitelist" << std::endl;
    // } else if (defendType == 1 && m_whitelist.find(m_srcAddr) == m_whitelist.end()) {
    //     // defend successfully
    //     m_defendSuccess = 2;
    //     std::cout << "Successful defense" << std::endl;
    // } else {
    //     m_defendSuccess = 1;
    //     std::cout << "failed defense" << std::endl;
    // }

    NS_LOG_INFO("ExecuteActions: defendType=" << defendType << ", defenseSuccess=" << m_defendSuccess);
    return true;
}
