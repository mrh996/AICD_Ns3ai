...
bool lastAction;
std::set<Ipv4Address> filterAddressList;
...
void ApplyAction(uint32_t action, ns3::Ipv4Address address) {
    const auto& whitelist = nge->GetWhitelist();
    // print addresses already in whitelist
    std::cout << "Current whitelist: ";
    for (const auto& addr : whitelist) {
        std::cout << addr << " ";
    }
    std::cout << std::endl;
    switch (action) {
        case 0: // stay
        std::cout<< " No defense performed;";
            NS_LOG_UNCOND("No defense performed: " << action);
            return;
        case 1: // Perform FW defense
        {   
            // uint32_t addressInt = Ipv4AddressToInt(address);
            // if (whitelist.find(addressInt) != whitelist.end()) {
            //     std::cout << " Address " << address << " is in whitelist, skipping addition to filter list." << std::endl;
            //     return;  // No insertion
            // }

            std::cout << " Push address: " << address << std::endl;
            filterAddressList.insert(address);
            for (const auto& addr : filterAddressList) {
                std::cout << " Current filter list: " << addr << " ";
            }
            std::cout << std::endl;
            return;
        }
...
void Monitor () {
    ...
    for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator fs = flowStats.begin(); fs != flowStats.end(); ++fs) {
        ...
        if (ft.destinationAddress == Ipv4Address(ipv4_address_server1)) {
            ...
            // Do defense
            ApplyAction(action, ft.sourceAddress);
        }
    }
    // Schedule after one second
    Simulator::Schedule(Seconds(0.01), &Monitor);
}
...
bool CustomReceiveCallback(Ptr<NetDevice> device, Ptr<const Packet> packet, uint16_t protocol, const Address &from)
{
    ...
    // Firewall
    for (const auto& filterAddress : filterAddressList)
    {   
        // std::cout <<  "Current filter address is " << Ipv4Address(filterAddress) << std::endl; 
        if (sourceAddress == filterAddress)
        {
            std::cout <<  "Packet from " << Ipv4Address(filterAddress) << " filtered" << std::endl;
            return true; 
        }
    }
